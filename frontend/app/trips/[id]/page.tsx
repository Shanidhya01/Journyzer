...
import ItineraryCard from "@/components/IteneraryCard";
import {
  ArrowLeft,
  Calendar,
  Clock,
...
function isActivitiesItinerary(itinerary: unknown): itinerary is ItineraryDayActivities[] {
  return (
...
function isSlotsItinerary(itinerary: unknown): itinerary is ItineraryDaySlots[] {
  return (
    Array.isArray(itinerary) &&
    itinerary.every(
      (d) =>
...
const EnhancedItineraryCard = ({ itinerary }: { itinerary: ItineraryDaySlots[] }) => (
  <div className="space-y-6">
    {itinerary.map((day) => (
      <div key={day.day} className="border-2 border-indigo-100 rounded-2xl overflow-hidden bg-gradient-to-br from-white to-indigo-50/30">
        ...
        <div className="p-6 space-y-4">
          <div className="flex gap-3">
            ...
            <div className="flex-1">
              <div className="text-xs font-bold text-amber-700 uppercase mb-1">Morning</div>
          ...
          <div className="flex gap-3">
            ...
            <div className="flex-1">
              <div className="text-xs font-bold text-orange-700 uppercase mb-1">Afternoon</div>
          ...
          <div className="flex gap-3">
            ...
            <div className="flex-1">
              <div className="text-xs font-bold text-indigo-700 uppercase mb-1">Evening</div>
...
export default function TripDetailsPage() {
  ...
  const locations = useMemo(() => (trip?.locations ?? []) as Location[], [trip]);

  const locationsByDay = useMemo(() => {
    const map: Record<number, Location[]> = {};
    for (const loc of locations) {
      const day = typeof loc.day === "number" && Number.isFinite(loc.day) ? loc.day : 1;
      if (!map[day]) map[day] = [];
      map[day].push(loc);
    }
    return map;
  ...
  const derivedDays = useMemo(() => {
    const fromTrip = typeof trip?.days === "number" && trip.days > 0 ? trip.days : 0;
    const fromLocations = locations.length > 0 ? Math.max(...locations.map((l) => (typeof l.day === "number" ? l.day : 1))) : 0;
    const fromItinerary = Array.isArray(trip?.itinerary) ? (trip?.itinerary as any[]).length : 0;
    return Math.max(fromTrip, fromLocations, fromItinerary, 1);
  }, [trip, locations]);

  const dayNumbers = useMemo(() => Array.from({ length: derivedDays }, (_, i) => i + 1), [derivedDays]);

  const selectedDayLocations = useMemo(() => locationsByDay[selectedDay] ?? [], [locationsByDay, selectedDay]);
      ...
      <div className="relative container mx-auto px-4 max-w-7xl py-8">
        ...
        {loading ? (
          <div className="bg-white/80 backdrop-blur-lg rounded-3xl shadow-2xl p-16 text-center border border-white/20">
            ...
            <p className="text-gray-700 font-semibold text-lg">Loading your adventure...</p>
          </div>
        ) : error ? (
            ...
            <div className="bg-white/80 backdrop-blur-lg rounded-3xl shadow-2xl overflow-hidden mb-8 border border-white/20">
              ...
              <div className="h-40 bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 relative overflow-hidden">
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <Plane className="w-20 h-20 text-white/30 animate-pulse" />
                </div>
                ...
                <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6">
                    ...
                    {Array.isArray(trip.interests) && trip.interests.length > 0 && (
                      <div className="bg-gradient-to-r from-indigo-50 to-purple-50 rounded-2xl p-4 border-2 border-indigo-100">
                        <div className="flex items-center gap-2 mb-3">
                          ...
                          <span className="text-sm font-bold text-indigo-900">Your Interests</span>
                        </div>
                  ...
                  <div className="flex flex-col gap-3">
                    <button
                      ...
                      disabled={generating}
                      className="bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 text-white px-8 py-4 rounded-2xl font-bold hover:shadow-2xl transition-all inline-flex items-center justify-center gap-3 disabled:opacity-50 disabled:cursor-not-allowed group relative overflow-hidden"
                    >
            ...
            <div className="grid lg:grid-cols-2 gap-8">
                  ...
                  {isSlotsItinerary(trip.itinerary) ? (
                    ...
                    <div className="space-y-6">
                      {trip.itinerary.map((day) => (
                        <div
                          key={day.day}
                          className="border-2 border-indigo-100 rounded-2xl overflow-hidden bg-gradient-to-br from-white to-indigo-50/30"
                        >
                          <div className="bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 px-6 py-4">
                            <h3 className="text-white font-bold text-lg flex items-center gap-2">
                              <Calendar className="w-5 h-5" />
                              Day {day.day}
                              {day.title && <span className="text-white/80">• {day.title}</span>}
                            </h3>
                          </div>
                          <div className="p-6">
                    ...
                    <div className="text-center py-12">
                      ...
                      <p className="text-gray-600 mb-1">Ready to start your adventure?</p>
                      <p className="text-sm text-gray-500">
                        Click <span className="font-semibold text-indigo-600">Generate Itinerary</span> to create your personalized plan
                      </p>
                    </div>
                  )}
                </div>
              ...
              <div className="space-y-6">
                <div className="flex items-center gap-3">
                  ...
                  <h2 className="text-2xl font-bold text-gray-900">Day-wise Maps</h2>
                </div>
                ...
                <div className="bg-white/80 backdrop-blur-lg rounded-3xl shadow-xl p-6 border border-white/20">
                  ...
                  <div className="mt-5">
                    <div className="flex items-center justify-between mb-3">
                      <h3 className="font-bold text-gray-900">Stops for Day {selectedDay}</h3>
                      {selectedDayLocations.length > 1 ? (
                        <button
                          onClick={() => {
                            const url = buildDayRouteUrl(selectedDayLocations);
                    ...
                    {selectedDayLocations.length === 0 ? (
                      <div className="bg-gray-50 border border-gray-200 rounded-2xl p-6 text-center">
                        <p className="text-gray-700 font-semibold">No saved locations for this day.</p>
                        <p className="text-sm text-gray-600">Generate itinerary to populate map points.</p>
                      </div>
                    ) : (
                            ...
                            >
                              <div className="flex items-start gap-3">
                                <div className="w-8 h-8 rounded-xl bg-indigo-100 flex items-center justify-center flex-shrink-0">
                                  <span className="text-sm font-extrabold text-indigo-700">{idx + 1}</span>
                                </div>
                                <div>
                                  <div className="font-bold text-gray-900">
                                    {loc.name || `Stop ${idx + 1}`}
                                  </div>
                                  <div className="text-xs text-gray-500">Tap to open directions</div>
                                </div>
                              </div>
                            </button>
